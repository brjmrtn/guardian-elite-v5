...
object BioController extends cask.Routes {
  ...
  def saveTraining(request: cask.Request) = withAuth(request) {
    ...
    val rpe    = params.getOrElse("rpe", "7").toIntOption.getOrElse(7)
    val calidad = params.getOrElse("calidad", "8").toIntOption.getOrElse(8)
    val att    = params.getOrElse("atencion", "8").toIntOption.getOrElse(8)
