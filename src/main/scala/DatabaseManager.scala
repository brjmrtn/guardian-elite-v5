...
object DatabaseManager {
  ...
  def logMatch(riv: String, gf: Int, gc: Int, min: Int, n: Double, med: Double, par: Int, zG: String, zT: String, zP: String, p1v1: Int, pAir: Int, pPie: Int, clima: String, temp: Int, notas: String, video: String, reaccion: String, fechaStr: String): Unit = {
    val conn=getConnection(); try {
      ...
      if(rs.next()){
        ...
        s.setString(15, clima); s.setInt(16, temp); s.setString(17, notas); s.setString(18, video); s.setString(19, reaccion); s.setDate(20, Date.valueOf(fechaStr))
        s.executeUpdate()
      }
  ...
  def logWellness(sueno: Int, energia: Int, dolor: Int, zona: String, altura: Int, peso: Double, animo: Int, notas: String): Unit = {
    val conn=getConnection(); try {
      ...
      s.setInt(1,sueno); s.setInt(2,energia); s.setInt(3,dolor); s.setString(4,zona); s.setInt(5, altura); s.setDouble(6, peso); s.setInt(7, animo); s.setString(8, notas)
      s.executeUpdate()
    } finally { conn.close() }
  }
  ...
  def logTraining(tipo: String, foco: String, rpe: Int, calidad: Int, atencion: Int): Unit = {
    val conn=getConnection(); try {
      ...
      s.executeUpdate(); conn.createStatement().executeUpdate("UPDATE gear SET usos_actuales = usos_actuales + 1 WHERE activo = TRUE")
    } finally { conn.close() }
  }
