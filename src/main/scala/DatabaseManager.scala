...
object DatabaseManager {
  ...
  // --- IA NEURO-SCOUT (AHORA CON PASES) ---
  def getDeepAnalysis(): String = {
    ...
    try {
      ...
      if(rsW.next()) {
        ...
        bioData = s"Calidad Sueño: ${rsW.getInt("sueno")}/5, HORAS: ${horas}h, Ánimo: ${rsW.getInt("animo")}/5."
        if(horas < 9.0) bioData += " (Déficit sueño)."
      }

      // DATOS DE DISTRIBUCIÓN RECIENTE
      var passStats = ""
      ...
      if(rsPass.next()) {
        ...
        if(st+lt > 0) passStats = s"Distribución (Últimos 3): Corto ${so}/${st}, Largo ${lo}/${lt}."
      }

      var drillInfo = ""; val activeDrills = getActiveDrills()
      if(activeDrills.nonEmpty) drillInfo = s"Foco técnico actual: ${activeDrills.map(_.nombre).mkString(", ")}."

      sb.append("Eres el entrenador de porteros top de Héctor. Analiza agenda, físico y juego de pies.\n")
      sb.append(s"1. AGENDA: Próximo vs $nextRival ($nextDate).\n")
      sb.append(s"2. FÍSICO: $estadoFisico. $bioData\n")
      sb.append(s"3. JUEGO DE PIES: $passStats\n")
      sb.append(s"4. ENTRENO: $drillInfo\n")

      sb.append("Responde en HTML bonito (fondo oscuro, letra 16px sans-serif). Título '🧠 ANÁLISIS DEL MÍSTER'.")
      sb.append("Si falla mucho en largo, aconséjale asegurar en corto. Si tiene buenos números, anímale a arriesgar.")
    ...
    } catch { case e: Exception => "<div style='color:#aaa'>Analizando datos tácticos...</div>" } finally { if(conn!=null) conn.close() }
  }
  ...
  def addNewDrill(nombre: String, desc: String): Unit = { val conn = getConnection(); try { val ps = conn.prepareStatement("INSERT INTO drills (nombre, descripcion, sesiones_objetivo, sesiones_actuales, activo) VALUES (?, ?, 10, 0, TRUE)"); ps.setString(1, nombre); ps.setString(2, desc); ps.executeUpdate() } finally { conn.close() } }
  ...
  def logWellness(sueno: Int, horas: Double, energia: Int, dolor: Int, zona: String, altura: Int, peso: Double, animo: Int, notas: String, estadoFisico: String): Unit = { val conn=getConnection(); try { val s=conn.prepareStatement("INSERT INTO wellness (sueno, horas_sueno, energia, dolor, zona_dolor, altura, peso, animo, notas_conducta, estado_fisico) VALUES (?,?,?,?,?,?,?,?,?,?)"); s.setInt(1,sueno); s.setDouble(2, horas); s.setInt(3,energia); s.setInt(4,dolor); s.setString(5,zona); s.setInt(6, altura); s.setDouble(7, peso); s.setInt(8, animo); s.setString(9, notas); s.setString(10, estadoFisico); s.executeUpdate() } finally { conn.close() } }
  ...
  // MATCH FUNCTIONS UPDATED WITH PASSES
  ...
  def playScheduledMatch(id: Int, gf: Int, gc: Int, min: Int, nota: Double, paradas: Int, notas: String, video: String, reaccion: String, clima: String, zonaGoles: String, zonaTiros: String, zonaParadas: String, p1v1: Int, pAir: Int, pPie: Int, pcTot: Int, pcOk: Int, plTot: Int, plOk: Int): Unit = {
    val conn=getConnection(); try{
      ...
      ps.setInt(1,gf); ps.setInt(2,gc); ps.setInt(3,min); ps.setDouble(4,nota); ps.setInt(5,paradas); ps.setString(6,notas); ps.setString(7,video); ps.setString(8,reaccion); ps.setString(9,clima); ps.setString(10,zonaGoles); ps.setString(11,zonaTiros); ps.setString(12,zonaParadas); ps.setInt(13,p1v1); ps.setInt(14,pAir); ps.setInt(15,pPie)
      ps.setInt(16,pcTot); ps.setInt(17,pcOk); ps.setInt(18,plTot); ps.setInt(19,plOk); ps.setInt(20,id)
      ps.executeUpdate()
    } finally {conn.close()}
  }

  // LOG NEW MATCH
  def logMatch(riv: String, gf: Int, gc: Int, min: Int, n: Double, med: Double, par: Int, zG: String, zT: String, zP: String, p1v1: Int, pAir: Int, pPie: Int, clima: String, temp: Int, notas: String, video: String, reaccion: String, fechaStr: String, pcTot: Int, pcOk: Int, plTot: Int, plOk: Int): Unit = {
    val conn=getConnection(); try{ val rs=conn.createStatement().executeQuery("SELECT MAX(id) as id FROM seasons"); if(rs.next()){
      val s=conn.prepareStatement("INSERT INTO matches (season_id, rival, goles_favor, goles_contra, minutos, nota, media_historica, paradas, zona_goles, zona_tiros, zona_paradas, paradas_1v1, paradas_aereas, acciones_pie, clima, temperatura, notas_partido, video_url, reaccion_goles, fecha, status, pc_t, pc_ok, pl_t, pl_ok) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'PLAYED',?,?,?,?)")
      s.setInt(1,rs.getInt("id")); s.setString(2,riv); s.setInt(3,gf); s.setInt(4,gc); s.setInt(5,min); s.setDouble(6,n); s.setDouble(7,med); s.setInt(8,par); s.setString(9,zG); s.setString(10,zT); s.setString(11,zP); s.setInt(12,p1v1); s.setInt(13,pAir); s.setInt(14,pPie); s.setString(15,clima); s.setInt(16,temp); s.setString(17,notas); s.setString(18,video); s.setString(19,reaccion); s.setDate(20,Date.valueOf(fechaStr))
      s.setInt(21,pcTot); s.setInt(22,pcOk); s.setInt(23,plTot); s.setInt(24,plOk)
      s.executeUpdate()
    } } finally {conn.close()}
  }

  def getRivalScouting(rivalBusqueda: String): (List[MatchLog], Map[String, Int]) = { var matches = List[MatchLog](); var stats = scala.collection.mutable.Map("pj"->0, "gf"->0, "gc"->0, "ganados"->0, "empatados"->0, "perdidos"->0); val conn = getConnection(); try { val query = s"SELECT id, rival, goles_favor, goles_contra, minutos, nota, TO_CHAR(fecha, 'DD/MM/YYYY') as f, clima, notas_partido, video_url, reaccion_goles, status, tipo_partido, pc_t, pc_ok, pl_t, pl_ok FROM matches WHERE LOWER(rival) LIKE LOWER(?) AND status='PLAYED' ORDER BY fecha DESC"; val stmt = conn.prepareStatement(query); stmt.setString(1, s"%$rivalBusqueda%"); val rs = stmt.executeQuery(); while(rs.next()) { val (gf, gc) = (rs.getInt("goles_favor"), rs.getInt("goles_contra")); val cl=Option(rs.getString("clima")).getOrElse(""); val nt=Option(rs.getString("notas_partido")).getOrElse(""); val vd=Option(rs.getString("video_url")).getOrElse(""); val re=Option(rs.getString("reaccion_goles")).getOrElse(""); matches = matches :+ MatchLog(rs.getInt("id"), rs.getString("rival"), s"$gf-$gc", rs.getInt("minutos"), rs.getDouble("nota"), rs.getString("f"), cl, nt, vd, re, rs.getString("status"), rs.getString("tipo_partido"), rs.getInt("pc_t"), rs.getInt("pc_ok"), rs.getInt("pl_t"), rs.getInt("pl_ok")); stats("pj")+=1; stats("gf")+=gf; stats("gc")+=gc; if(gf>gc) stats("ganados")+=1 else if(gf==gc) stats("empatados")+=1 else stats("perdidos")+=1 } } finally { conn.close() }; (matches, stats.toMap) }
