...
// NUEVOS MODELOS v3
case class GrowthEntry(fecha: String, altura: Double, peso: Double, vel: Double)
...
object DatabaseManager {
  ...
  // --- IA NEURO-SCOUT v3.0 (AHORA CON FACTOR CRECIMIENTO) ---
  def getDeepAnalysis(): String = {
    ...
    try {
      ...
      if(rsW.next()) {
        ...
        bioData = s"Sueño: ${rsW.getDouble("horas_sueno")}h, Ánimo: ${rsW.getInt("animo")}/5."
      }

      // CHECK CRECIMIENTO (COORDINACIÓN)
      var growthAlert = ""
      val rsG = conn.createStatement().executeQuery("SELECT velocidad_crecimiento FROM physical_growth ORDER BY fecha DESC LIMIT 1")
      if(rsG.next() && rsG.getDouble("velocidad_crecimiento") > 0.5) {
        growthAlert = s"⚠️ ALERTA BIOMÉTRICA: Ha crecido ${rsG.getDouble("velocidad_crecimiento")}cm este mes. Puede tener pérdida temporal de coordinación. Simplificar acciones."
      }
      ...
      if(rsPass.next()) {
        val (st, so, lt, lo) = (rsPass.getInt("st"), rsPass.getInt("so"), rsPass.getInt("lt"), rsPass.getInt("lo"))
        if(st+lt > 0) passStats = s"Pases (Últimos 3): Corto ${so}/${st}, Largo ${lo}/${lt}."
      }
      ...
      sb.append("Responde en HTML bonito (fondo oscuro, letra 16px). Título '🧠 ANÁLISIS DEL MÍSTER'. Si hay alerta de crecimiento, menciónala prioritariamente.")
    ...
    } catch { case e: Exception => "<div style='color:#aaa'>Cargando...</div>" } finally { if(conn!=null) conn.close() }
  }

  // --- BIO-GROWTH FUNCTIONS ---
  def logGrowth(altura: Double, peso: Double): Unit = {
    val conn = getConnection()
    try {
      // Calcular velocidad crecimiento vs mes anterior
      var velocity = 0.0
      val rsLast = conn.createStatement().executeQuery("SELECT altura FROM physical_growth ORDER BY fecha DESC LIMIT 1")
      if(rsLast.next()) {
        val lastHeight = rsLast.getDouble("altura")
        if(altura > lastHeight) velocity = altura - lastHeight
      }

      val ps = conn.prepareStatement("INSERT INTO physical_growth (altura, peso, velocidad_crecimiento) VALUES (?, ?, ?)")
  ...
  def getGrowthHistory(): String = {
    var l = List[String](); var d = List[Double]()
    val conn = getConnection()
    try {
      val rs = conn.createStatement().executeQuery("SELECT TO_CHAR(fecha, 'MM-DD') as f, altura FROM physical_growth ORDER BY fecha ASC LIMIT 12")
      while(rs.next()) { l=l:+s"'${rs.getString("f")}'"; d=d:+rs.getDouble("altura") }
    } finally { conn.close() }
    s"""{ "labels": [${l.mkString(",")}], "data": [${d.mkString(",")}] }"""
  }

  // --- VIDEO TAGGING FUNCTIONS ---
  def addVideoTag(matchId: Int, min: Int, sec: Int, tipo: String, desc: String): Unit = {
    val conn = getConnection()
    try {
      val ps = conn.prepareStatement("INSERT INTO video_tags (match_id, minuto, segundo, tipo, descripcion) VALUES (?,?,?,?,?)")
      ps.setInt(1, matchId); ps.setInt(2, min); ps.setInt(3, sec); ps.setString(4, tipo); ps.setString(5, desc)
      ps.executeUpdate()
    } finally { conn.close() }
  }

  def getVideoTags(matchId: Int): List[VideoTag] = {
    var l = List[VideoTag](); val conn = getConnection()
    try {
      val rs = conn.createStatement().executeQuery(s"SELECT * FROM video_tags WHERE match_id = $matchId ORDER BY minuto ASC, segundo ASC")
      while(rs.next()) l=l:+VideoTag(rs.getInt("id"), rs.getInt("match_id"), rs.getInt("minuto"), rs.getInt("segundo"), rs.getString("tipo"), rs.getString("descripcion"))
    } finally { conn.close() }
    l
  }
  ...
  // --- RIVALS FUNCTIONS (Basic) ---
  def getRivalInfo(nombre: String): Option[RivalInfo] = {
    var r: Option[RivalInfo] = None
    val conn = getConnection()
    try {
      val ps = conn.prepareStatement("SELECT * FROM rivals WHERE LOWER(nombre) = LOWER(?)")
      ps.setString(1, nombre)
      val rs = ps.executeQuery()
      if(rs.next()) r = Some(RivalInfo(rs.getString("nombre"), rs.getString("estilo_juego"), rs.getString("jugadores_clave"), rs.getString("notas_scouting")))
    } finally { conn.close() }
    r
  }

  def saveRivalInfo(nombre: String, estilo: String, claves: String, notas: String): Unit = {
    val conn = getConnection()
    try {
      // Upsert simple (borrar e insertar para no complicar con updates)
      conn.createStatement().executeUpdate(s"DELETE FROM rivals WHERE LOWER(nombre) = LOWER('${nombre}')")
      val ps = conn.prepareStatement("INSERT INTO rivals (nombre, estilo_juego, jugadores_clave, notas_scouting) VALUES (?,?,?,?)")
      ps.setString(1, nombre); ps.setString(2, estilo); ps.setString(3, claves); ps.setString(4, notas)
      ps.executeUpdate()
    } finally { conn.close() }
  }
  ...
  def addNewDrill(n: String, d: String): Unit = { val conn=getConnection(); try{ val ps=conn.prepareStatement("INSERT INTO drills (nombre, descripcion, sesiones_objetivo, sesiones_actuales, activo) VALUES (?, ?, 10, 0, TRUE)"); ps.setString(1, n); ps.setString(2, d); ps.executeUpdate() } finally {conn.close()} }
