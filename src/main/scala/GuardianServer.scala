...
object GuardianServer extends cask.MainRoutes {
  ...
  @cask.get("/bio")
  def bioPage() = {
    val content = basePage("bio", div(cls := "row justify-content-center", div(cls := "col-md-6 mb-4", div(cls := "card bg-dark text-white border-info shadow", div(cls := "card-header bg-info text-dark fw-bold text-center", "WELLNESS & NEURO"), div(cls := "card-body p-3", form(action := "/bio/save_wellness", method := "post", div(cls:="row mb-3", div(cls:="col-6 text-center", label(cls:="small", "Sueno (1-5)"), input(tpe:="range", cls:="form-range", min:="1", max:="5", name:="sueno")), div(cls:="col-6 text-center", label(cls:="small", "Energia (1-5)"), input(tpe:="range", cls:="form-range", min:="1", max:="5", name:="energia"))),
      // NUEVO: ESTADO DE ÁNIMO
      div(cls:="mb-3 border-top pt-2", label(cls:="small text-info fw-bold", "Estado Animico / Regulacion (1-5)"), input(tpe:="range", cls:="form-range", min:="1", max:="5", name:="animo", id:="animoRange"), div(cls:="d-flex justify-content-between xx-small text-muted", span("Crisis"), span("Regulado"))),
      div(cls:="mb-2", input(tpe:="text", name:="notas_conducta", cls:="form-control form-control-sm bg-dark text-white border-secondary", placeholder:="Notas conducta (opcional)")),
      div(cls:="mb-3", label(cls:="small", "Dolor?"), select(name:="dolor", cls:="form-select", option(value:="1", "Nada"), option(value:="2", "Molestia"), option(value:="3", "Dolor"), option(value:="5", "Lesion"))), div(cls:="mb-3", input(tpe:="text", name:="zona", cls:="form-control", placeholder:="Zona dolor (opcional)")),
      ...
      div(cls:="d-grid", button(tpe:="submit", cls:="btn btn-outline-info", "Guardar Bio"))))), div(cls := "col-md-6", div(cls := "card bg-dark text-white border-success shadow", div(cls := "card-header bg-success text-dark fw-bold text-center", "ENTRENO"), div(cls := "card-body p-3", form(action := "/bio/save_training", method := "post", div(cls:="mb-3", select(name:="tipo", cls:="form-select", option(value:="Club", "Club"), option(value:="Academia", "Academia"), option(value:="Papa", "Papa"))), div(cls:="mb-3", input(tpe:="text", name:="foco", cls:="form-control", placeholder:="Foco (ej: Blocaje)")),
      // NUEVO: NIVEL DE ATENCIÓN
      div(cls:="mb-3 bg-secondary bg-opacity-25 p-2 rounded", label(cls:="small text-white fw-bold", "Nivel de Atencion / Foco (1-5)"), input(tpe:="range", cls:="form-range", min:="1", max:="5", name:="atencion", value:="3"), div(cls:="d-flex justify-content-between xx-small text-muted", span("Distraido"), span("Hiper-Foco"))),
      div(cls:="row mb-3", div(cls:="col-6 text-center", label(cls:="small", "Esfuerzo"), input(tpe:="number", cls:="form-control text-center", name:="rpe", value:="7")), div(cls:="col-6 text-center", label(cls:="small", "Calidad"), input(tpe:="number", cls:="form-control text-center", name:="calidad", value:="8"))), div(cls:="d-grid", button(tpe:="submit", cls:="btn btn-outline-success", "Guardar")))))))))
      cask.Response(content.getBytes("UTF-8"), headers = Seq("Content-Type" -> "text/html; charset=utf-8"))
  }

  @cask.postForm("/bio/save_wellness") def saveWellness(sueno: Int, energia: Int, dolor: Int, zona: String, altura: String, peso: String, animo: Int, notas_conducta: String) = {
    val altVal = if(altura.nonEmpty) altura.toInt else 0; val pesoVal = if(peso.nonEmpty) peso.toDouble else 0.0
    DatabaseManager.logWellness(sueno, energia, dolor, zona, altVal, pesoVal, animo, notas_conducta)
  ...
  @cask.postForm("/bio/save_training") def saveTraining(tipo: String, foco: String, rpe: Int, calidad: Int, atencion: Int) = { DatabaseManager.logTraining(tipo, foco, rpe, calidad, atencion); val htmlStr = doctype("html")(html(head(meta(charset:="utf-8"), tags2.style(raw(getCss()))), body(style:="background:#1a1a1a;color:white;text-align:center;padding-top:50px;font-family:'Oswald';", h1("OK"), h3("Guardado"), div(style:="margin-top:20px;", a(href:="/bio", cls:="btn btn-success", "Volver"))))).render; cask.Response(htmlStr.getBytes("UTF-8"), headers = Seq("Content-Type" -> "text/html; charset=utf-8")) }

  // 4. HISTORY (IGUAL QUE ANTES)
  @cask.get("/history")
  def historyPage() = {
