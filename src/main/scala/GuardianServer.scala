...
object GuardianServer extends cask.MainRoutes {
  ...
  def dashboard() = {
    ...
    val logros = DatabaseManager.getAchievements(); val aiMessage = DatabaseManager.getAdvancedAiAnalysis(); val tac = DatabaseManager.getTacticalStats(); val objs = DatabaseManager.getSeasonObjectives()
    ...
    val trendDiff = avgLast5 - avgSeason; val trendText = if (trendDiff > 0.5) "MEJORA" else if (trendDiff < -0.5) "BAJA" else "IGUAL"; val trendColor = if (trendDiff > 0) "text-success" else if (trendDiff < 0) "text-danger" else "text-muted"
  ...
  // 2. MATCH CENTER (AÑADIDO CAMPO FECHA)
  ...
  def saveMatch(rival: String, gf: Int, gc: Int, minutos: Int, nota: Double, paradas: Int, zonaGoles: String, zonaTiros: String, zonaParadas: String, p1v1: Int, pAir: Int, pPie: Int, clima: String, temp: Int, notas: String, video: String, reaccion: String, fecha: String, mode: String) = {
    ...
    DatabaseManager.logMatch(rival, gf, gc, minutos, nota, n.media, paradas, zonaGoles, zonaTiros, zonaParadas, p1v1, pAir, pPie, clima, temp, notas, video, reaccion, fecha)
  ...
  @cask.postForm("/bio/save_wellness") def saveWellness(sueno: Int, energia: Int, dolor: Int, zona: String, altura: String, peso: String) = {
    val altVal = if(altura.nonEmpty) altura.toInt else 0; val pesoVal = if(peso.nonEmpty) peso.toDouble else 0.0
    DatabaseManager.logWellness(sueno, energia, dolor, zona, altVal, pesoVal)
    val htmlStr = doctype("html")(html(head(meta(charset:="utf-8"), tags2.style(raw(getCss()))), body(style:="background:#1a1a1a;color:white;text-align:center;padding-top:50px;font-family:'Oswald';", h1("OK"), h3("Guardado"), div(style:="margin-top:20px;", a(href:="/bio", cls:="btn btn-info", "Volver"))))).render
    cask.Response(htmlStr.getBytes("UTF-8"), headers = Seq("Content-Type" -> "text/html; charset=utf-8"))
  }
  ...
  @cask.postForm("/match/update") def updateMatchAction(id: Int, rival: String, gf: Int, gc: Int, nota: Double, notas: String, video: String, reaccion: String, fecha: String) = { DatabaseManager.updateMatch(id, rival, gf, gc, 60, nota, "Sol", 20, notas, video, reaccion, fecha); cask.Response("".getBytes("UTF-8"), statusCode = 302, headers = Seq("Location" -> "/history")) }

  // 5. ADMIN, SETTINGS... (Igual que antes)
  ...
  def importerPage() = {
    val content = basePage("settings", div(cls:="row justify-content-center", div(cls:="col-md-8",
      h2(cls:="text-info text-center mb-4", "IMPORTADOR DE DATOS"),
      div(cls:="card bg-dark text-white border-warning shadow p-4 mb-4",
        h4("Importar Partidos (CSV)"),
        p(cls:="small text-muted", "Copia y pega desde Excel con este orden exacto:"),
        div(cls:="alert alert-secondary p-2 small", "Rival, GF, GC, Min, Nota, Paradas, Clima, Notas"),
        form(action:="/admin/upload_matches", method:="post",
          textarea(name:="csvContent", cls:="form-control mb-3", rows:="5", placeholder:="Rayo, 2, 1, 60, 8.5, 6, Lluvia, Partido intenso..."),
          button(tpe:="submit", cls:="btn btn-warning w-100", "Procesar Partidos")
        )
      ),
      div(cls:="card bg-dark text-white border-info shadow p-4",
        h4("Importar Wellness (CSV)"),
        div(cls:="alert alert-secondary p-2 small", "Sueno(1-5), Energia(1-5), Dolor(1-5), Zona"),
        form(action:="/admin/upload_wellness", method:="post",
          textarea(name:="csvContent", cls:="form-control mb-3", rows:="5", placeholder:="3, 4, 1, Nada..."),
          button(tpe:="submit", cls:="btn btn-info w-100", "Procesar Wellness")
        )
      ),
